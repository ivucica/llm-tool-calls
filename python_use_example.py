...
# Set destrictify to true for Gemini models
# This is a workaround for the Gemini API, which doesn't like strict=True in the function tool definition.
destrictify = 'generativelanguage.googleapis.com' in base_url  # set to true for gemini
...
def get_model_list() -> list[str]:
    ...
    try:
        ...
        models: typing.Iterable[openai.types.Model]
...
def chat_loop(conversation: Conversation):
    ...
    while True:
        ...
        if user_input.startswith("/model "):
            new_m = user_input[len("/model "):]
            if new_m == 'default':
                conversation.active_model = None
                print("Model set to default")
                conversation.save_history("default_history.json")
                continue
            elif new_m not in get_model_list():
                print(f"Model '{new_m}' not found. Available models: {get_model_list()}")
                continue

            conversation.active_model = user_input[len("/model "):]
            print(f"Model set to {conversation.active_model}")
            conversation.save_history("default_history.json")
            continue

        if user_input.startswith("/model"):
            if conversation.active_model:
                print(f"Model: {conversation.active_model}")
                continue
            else:
        ...
        try:
            ...
            replacement_messages = ask(
                model=conversation.active_model or MODEL,
                messages=messages,
